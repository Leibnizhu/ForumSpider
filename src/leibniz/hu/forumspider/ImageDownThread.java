...
public class ImageDownThread implements Runnable {
	...
	public void run() {
		/*synchronized(threadNum){
			ImageDownThread.threadNum++;
		}*/
		Map<String, String> tempMission = null;
		/*if(ImageDownThread.threadNum%10 == 0){
			System.out.println(new Date() + " 图片下载启动------------------>像我这么屌的图片下载器有" + ImageDownThread.threadNum + "个");
		}*/
		while(true){
			//待办任务大于帖子解析器数量N倍则开启新线程
			/*while((Spider.imageDownList.size() / ImageDownThread.threadNum) >= 2){
			...
			/*if(Spider.imageDownList.size() / ImageDownThread.threadNum < 0.2 && ImageDownThread.threadNum > 20){
		...
		if(ImageDownThread.threadNum%10 == 0){
			System.out.println(new Date() + " 图片下载器关闭-------------------------->当前运行的图片下载器有" + ImageDownThread.threadNum + "个");
		}*/
	}
}
